...
class App(ctk.CTk):
    ...
    def update_bmi(self, *args):
        ...
        weight_kilo = self.weight_float.get()
        bmi_result = round(weight_kilo / height_meter ** 2, 2)
...
class WeightInput(ctk.CTkFrame):
    def __init__(self,parent, weight_float, metric_bool):
        ...
        label = ctk.CTkLabel(self, text_color=BLACK, font=font, textvariable = self.output_string)
        label.grid(row = 0, column = 2)
        
        #buttons
        minus_button = ctk.CTkButton(self, command=lambda:self.update_weight(('minus', 'large')), text='-',fg_color=LIGHT_GRAY, hover_color=GRAY, text_color=BLACK, font=font, corner_radius=BUTTON_CORNER_RADIUS)
        minus_button.grid(row = 0, column = 0, sticky = 'ns', padx = 8, pady = 8)
        
        plus_button = ctk.CTkButton(self, command=lambda:self.update_weight(('plus', 'large')), text='+',fg_color=LIGHT_GRAY, hover_color=GRAY, text_color=BLACK, font=font, corner_radius=BUTTON_CORNER_RADIUS)
        plus_button.grid(row = 0, column = 4, sticky = 'ns', padx = 8, pady = 8)
        
        small_plus_button = ctk.CTkButton(self, command=lambda:self.update_weight(('plus', 'small')), text='+',fg_color=LIGHT_GRAY, hover_color=GRAY, text_color=BLACK, font=font, corner_radius=BUTTON_CORNER_RADIUS)
        small_plus_button.grid(row = 0, column = 3, padx = 4, pady = 4)
        
        small_minus_button = ctk.CTkButton(self, command=lambda:self.update_weight(('minus', 'small')), text='-',fg_color=LIGHT_GRAY, hover_color=GRAY, text_color=BLACK, font=font, corner_radius=BUTTON_CORNER_RADIUS)
